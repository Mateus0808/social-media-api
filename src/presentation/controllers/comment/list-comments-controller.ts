...
export class ListCommentsController implements Controller {
  constructor (private readonly listCommentsService: ListCommentsServiceInterface) {}
    ...
    try {
      ...
      for (const param of possibleParams) {
        if (httpRequest.queryParams[param]) {
          if (httpRequest.queryParams[param] === 'undefined' || httpRequest.queryParams[param] === undefined) {
      ...
      const response = await this.listCommentsService.listComments({ page, limit })
      return ok(response)
    } catch (error: any) {
