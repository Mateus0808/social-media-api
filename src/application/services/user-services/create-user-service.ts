...
import { CreateUserParams, CreateUserResponse, CreateUserServiceInterface } from '../../interfaces/create-user-service-interface'
...
export class CreateUserService implements CreateUserServiceInterface {
  ...
  async createUser (createUserParams: CreateUserParams): Promise<CreateUserResponse> {
    const { email } = createUserParams
    const userAlreadyExists = await this.loadUserByEmailRepository.loadByEmail(email)
