...
import { AuthUserServiceInterface, AuthUserParams, AuthUserResponse } from '../../interfaces/auth-user-service-interface'
...
export class AuthUserService implements AuthUserServiceInterface {
  ...
  async auth (authUserParams: AuthUserParams): Promise<AuthUserResponse> {
    ...
    const response: AuthUserResponse = { user: userCreatedDto(user), token: token }
